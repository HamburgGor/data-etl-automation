...
- Bash 一站式自动化运维：支持服务器环境一键初始化、systemd 系统服务自动创建、cron 定时任务自动注册，实现服务健康巡检与定时重启。
- 数据生命周期自动化管理：定时自动生成带日期戳的测试演示数据，同时自动清理超5天的过期旧文件，无需人工维护。
📁 完整项目结构
整体采用分层架构设计，拆分业务层、工具层、运维脚本层，结构清晰、便于拓展与维护。
data-etl-automation/
├── .gitignore                # Git 忽略配置
├── README.md                 # 项目说明文档
├── requirements.txt          # Python 依赖清单
├── main.py                   # ETL 程序主入口
├── gen_test_data.py          # 生成50万行测试脏数据（压力测试）
├── gen_daily_data.py         # 定时日常演示数据生成脚本
├── core/                     # 公共工具底层（通用能力封装）
│   ├── __init__.py
...
├── tests/                    # 测试用例目录（预留）
└── __init__.py
自动生成目录：项目运行后将自动创建 demo_data/（测试数据）、output/（处理结果）、logs/（运行日志），无需手动新建。
🚀 快速开始（本地开发环境）
1. 环境要求
- Python 3.8+
- pip 包管理工具
...
- 注册 etl_auto_main 系统服务，支持崩溃自动重启
- 配置多项 cron 定时任务：
...
📊 进度展示与日志体系
- 终端实时进度：彩色动态进度条，实时刷新处理行数、完成百分比，无刷屏干扰
- 项目运行日志：logs/etl_run.log 完整记录每一次ETL任务的执行流程、耗时、报错信息
